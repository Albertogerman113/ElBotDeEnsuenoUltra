    ...
    if activar and api_key and api_secret:
        try:
            ...
            for p in posiciones:
                ...
                pos_html += (
                    f'<hr style="border-color:{color};opacity:0.3;margin:6px 0">'
                    f'<b style="color:{color}">{sym.split("/")[0]} {side}</b> {trailing}{be}<br>'
                    f'<span style="color:#aaa;font-size:0.88em">'
                    f'@{entry:.2f} | SL:{sl:.2f} | TP:{tp:.2f}<br>'
                    f'PnL:${pnl:+.4f} | Mark:{mark:.2f} | MFE:{mfe:.2f}%</span>'
                )
            
            if pos_html:
                pos_full = f'<b>📈 Posiciones ({n_activas}/{Config.MAX_POSITIONS})</b><br>{pos_html}'
            else:
                pos_full = f'<b>📈 Posiciones (0/{Config.MAX_POSITIONS})</b><br><span style="color:#667799">Sin posiciones activas</span>'
            
            posicion_ph.markdown(
                f'<div class="metric-card">{pos_full}</div>',
                unsafe_allow_html=True
            )
            ...
            # UI Señales (sin divs anidados - Streamlit no los renderiza)
            senales_html = ""
            for s in senales_encontradas:
                ...
                senales_html += (
                    f'<hr style="border-color:{color};opacity:0.4;margin:8px 0">'
                    f'<span style="color:{color};font-weight:700;font-size:1.05em">'
                    f'{s["side"].upper()} {sym_name}</span>'
                    f' <span style="color:#8899aa">Score:{s["score"]:.1f} ADX:{s.get("adx", 0):.0f}</span><br>'
                    f'<span style="color:#cccccc;font-size:0.9em">'
                    f'@{s["entry"]:.2f} | SL:{s["sl"]:.2f} | TP:{s["tp"]:.2f} | RR:{Config.RR_RATIO}:1</span><br>'
                    f'<span style="color:#8899bb;font-size:0.82em">{razones}</span>'
                )
            
            if senales_html:
                full_html = f'<b>🎯 Señales ({len(senales_encontradas)})</b><br>{senales_html}'
            else:
                full_html = '<b>🎯 Señales (0)</b><br><span style="color:#667799">Escaneando...</span>'
            
            senal_ph.markdown(
                f'<div class="metric-card">{full_html}</div>',
                unsafe_allow_html=True
            )
            
            # Logs (sin divs anidados)
            log_entries = logger.get_logs(30)
            if log_entries:
                log_html = "<br>".join([f'<span style="font-family:monospace;font-size:0.78em;color:#99aabb">{l}</span>' for l in log_entries])
            else:
                log_html = '<span style="color:#667799">Sin logs</span>'
            
            log_ph.markdown(
                f'<div class="metric-card" style="max-height:350px;overflow-y:auto">{log_html}</div>',
                unsafe_allow_html=True
            )
