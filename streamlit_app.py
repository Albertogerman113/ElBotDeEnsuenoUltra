...
class SmartMoneyAnalyzer:
    ...
    def detect_market_structure(self, lookback: int = 50) -> Tuple[MarketStructure, Optional[float], Optional[float]]:
        ...
        # Estructura mixta
            ...
            structure = MarketStructure.BEARISTIC
...
class SignalGenerator:
    ...
    def generate_signal(self, symbol: str) -> Optional[TradeSignal]:
        ...
        elif self.structure_1h == MarketStructure.BEARISTIC:
            score_short += 3
        ...
        elif self.structure_15m == MarketStructure.BEARISTIC:
            score_short += 1
...
class StatsManager:
    ...
    def update_trade(stats: dict, pnl: float, reason: str) -> dict:
        ...
        # Añadir al historial
        stats['trades_history'].append({
            'date': datetime.now().isoformat(),
            'pnl': pnl,
            'reason': reason
        })
        
        # Mantener solo los últimos 100 trades en historial
        ...
        try:
            ...
            while True:
                ...
                <div class="metric-card">
                    ...
                    <small>W: {stats['wins']} | L: {stats['losses']} | BE: {stats['breakeven']}</small><br>
                    <small>PnL Total: <span style="color:{"#00ff88" if stats['total_pnl'] >= 0 else "#ff4466"}">${stats['total_pnl']:.4f}</span></small><br>
                    <small>Win Rate: {stats['win_rate']:.1f}% | Racha: {stats['current_streak']}</small>
                </div>
                """, unsafe_allow_html=True)
